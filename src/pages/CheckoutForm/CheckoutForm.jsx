...
const CheckoutForm = () => {
  ...
  }, [formData.email,formData.name,formData.address,formData.phoneNo,cart.length,]);
  ...
  const handleSubmit = async () => {
    ...
    if (formData.email && validator.isEmail(formData.email) && formData.name && formData.address && formData.phoneNo && cart.length !== 0) {
      setIsLoading(true)
      const result = await fetch("https://better-wear-pike.cyclic.app/ship/checkout", {
        method: "POST",
      ...
      setValidateMsg({error: true, msg: "All field are required or Cart is Empty",});
  ...
  return (
    <div className="checkoutContainer">
      ...
      <div className="checkout-form">
        ...
        <input type="text" name="name" value={formData.name} id="name" placeholder="Name" onChange={handleFormData} />
        <input type="email" name="email" value={formData.email} id="email" placeholder="Email" onChange={handleFormData}/>
        {emailError.status && <Alert severity="error">{emailError.msg}</Alert>}
        <textarea name="address" id="" cols="10" value={formData.address} rows="2" placeholder="Enter your Address" onChange={handleFormData}/>
        <input type="text" name="phoneNo" value={formData.phoneNo} id="phoneNo" placeholder="Phone Number" onChange={handleFormData}/>
        {isLoading?<Loader/>:<input className="btn" type="button" value="Confirm your order" onClick={handleSubmit}/>}
